...
export class Weather extends PureComponent {
  ...
  fetchLocationInfo = async (event) => {
    ...
    try {
      ...
      if (searchText.length > 2) {
        ...
        this.setState({
          cities: json.filter((v, i, a) => a.findIndex(t => (t.name.toLowerCase() === v.name.toLowerCase() && t.country.toLowerCase() === v.country.toLowerCase())) === i),
        });
      } else {
  ...
  render() {
    ...
    const {
      report, cities, httpStatus, units,
    } = this.state;
    const {
      loadDataStatus, loadCityStatus, defaultStatus, changeUnitStatus,
